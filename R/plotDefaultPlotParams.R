...
plotDefaultPlotParams <- function(plot.type=2) {
  ...
  if(plot.type == 2) { #Horizontal. Data above and below the ideogram
    ...
    #outmargins
    kp$beginKpPlot()
      abline(h=c(kp$plot.params$bottommargin, kp$plot.params$bottommargin + kp$chromosome.height, kp$plot.params$bottommargin + 2*kp$chromosome.height))
      marg.x <- kp$plot$xmax /10 * 8
      arrows(x0 = marg.x, x1=marg.x, y0=0, y1=kp$plot.params$bottommargin, code=3, length=0.07)
      text(x=marg.x, y=kp$plot.params$bottommargin/2,labels=paste0("bottommargin=", kp$plot.param$bottommargin), pos=2)
      
      arrows(x0 = marg.x, x1=marg.x, y0=kp$plot$ymax, y1=kp$plot$ymax - kp$plot.params$topmargin, code=3, length=0.07)
      text(x=marg.x, y=kp$plot$ymax - kp$plot.params$topmargin/2, labels=paste0("topmargin=", kp$plot.param$topmargin), pos=2)
      
      ch.x <- kp$plot$xmax /10 * 9.2
      arrows(x0 = ch.x, x1=ch.x, y0=kp$plot.params$bottommargin + kp$chromosome.height, y1=kp$plot.params$bottommargin + 2*kp$chromosome.height, code=3, length=0.07)
      text(x=ch.x, y=kp$plot.params$bottommargin + 1.5*kp$chromosome.height, labels=paste0("chromosome.height (computed)=", kp$chromosome.height), pos=2)
      
      abline(v=c(kp$plot.params$leftmargin, kp$plot$xmax-kp$plot.params$rightmargin), lty=2)
      marg.y <- kp$plot$ymax /10 * 9.4
      arrows(x0 = 0, x1=kp$plot.params$leftmargin, y0=marg.y, y1=marg.y, code=3, length=0.07)
      text(x=kp$plot.params$leftmargin/2, y=marg.y, labels=paste0("leftmargin=", kp$plot.params$leftmargin), pos=3)
      
      arrows(x0 = kp$plot$xmax - kp$plot.params$rightmargin, x1=kp$plot$xmax, y0=marg.y, y1=marg.y, code=3, length=0.07)
      text(x= kp$plot$xmax - kp$plot.params$rightmargin/2, y=marg.y, labels=paste0("rightmargin=", kp$plot.params$rightmargin), pos=3)
