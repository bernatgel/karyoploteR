...
<<fig.height=6,fig.width=8>>=
  gains <- data.frame(chr=c("chr1", "chr5", "chr17", "chr22"), start=c(1, 1000000, 4000000, 1),
                      end=c(5000000, 3200000, 80000000, 1200000))
  losses <- data.frame(chr=c("chr3", "chr9", "chr17"), start=c(80000000, 20000000, 1),
...
<<fig.height=4,fig.width=8>>=
  ...
  kpText(kp, chr=sel.data$chr, x=sel.data$start, y=sel.data$y,
         ymin=-1, ymax=1, r0=0.05, r1=0.4, labels=c("A", "B", "C", "D"), col="red", pos=4, cex=0.8)
  ...
  kpDataBackground(kp, r0=0, r1=0.3, color = "#EEFFEE", data.panel = 2)
  kpAxis(kp, col="#AADDAA", ymin=-1, ymax=1, r0=0, r1=0.3, data.panel = 2, numticks = 2, cex=0.5, tick.len = 0)
  kpAbline(kp, h=0, col="#AADDAA", ymin=-1, ymax=1, r0=0, r1=0.3, data.panel = 2)
  kpBars(kp, chr=rand.data$chr, x0=rand.data$start, x1=rand.data$end, y1 = rand.data$y,
          col="#AADDAA", ymin=-1, ymax=1, r0=0, r1=0.3, data.panel = 2, border="#AADDAA" )
  
  kpDataBackground(kp, r0=0.33, r1=0.63, color = "#EEEEFF", data.panel = 2)
  kpAxis(kp, col="#AAAADD", ymin=-1, ymax=1, r0=0.33, r1=0.63, data.panel = 2, numticks = 2, cex=0.5, tick.len = 0)
  kpAbline(kp, h=0, col="#AAAADD", ymin=-1, ymax=1, r0=0.33, r1=0.63, data.panel = 2)
  kpArrows(kp, chr=rand.data$chr, x0=rand.data$start, x1=rand.data$end, y0=0, y1=rand.data$y,
          col="#AAAADD", ymin=-1, ymax=1, r0=0.33, r1=0.63, data.panel = 2, lwd=2, length=0.05)
  
  kpDataBackground(kp, r0=0.66, r1=0.96, color = "#FFEEEE", data.panel = 2)
  kpAxis(kp, col="#DDAAAA", ymin=-1, ymax=1, r0=0.66, r1=0.96, data.panel = 2, numticks = 2, cex=0.5, tick.len = 0)
  kpPoints(kp, chr=rand.data$chr, x=rand.data$start, y=rand.data$y,
          col="#DDAAAA", ymin=-1, ymax=1, r0=0.66, r1=0.96, data.panel = 2, pch=".", cex=5)
...
There are currently 2 plot types available: 

\begin{description}
  \item[plot.type=1] A plot with horizontal ideograms and a single data panel on top of them
  \item[plot.type=2] A plot with horizontal ideograms and two data panels, one above and one below them
\end{description}
...
This is plot type 1:
<<fig.height=2,fig.width=8>>=
  kp <- plotKaryotype(chromosomes = c("chr1"), plot.type = 1)
  kpDataBackground(kp, data.panel = 1)
@

And this is plot type 2:
<<fig.height=2,fig.width=8>>=
 kp <- plotKaryotype(chromosomes = c("chr1"), plot.type = 2)
 kpDataBackground(kp, data.panel = 1)
...
<<fig.height=6,fig.width=8>>=
  ...
  kpAxis(kp, r0=0.53, tick.pos = c(0.3, 0.6, 1), labels = c("A", "B", "C"), col="#66AADD", cex=0.5, data.panel=2)
...
% This are the available parameters and their default values:
% 
% \begin{center}\vspace{1cm}
%   \includegraphics{./figures/plot.params.type2.png}
%   \captionof{figure}{Plotting parameters available}
% \end{center}\vspace{1cm}

The easiest way to get a valid \Rcode{plot.params} object is calling \Rfunction{getDefaultPlotParams} and modifying the response.

For example, we can create a plot with a data plot larger than the other and smaller margins:
...
Or a plot with a smaller single data panel toucing the ideogram, and add the base numbers and cytoband names:
...
kp <- plotKaryotype(chromosomes = c("chr1", "chr2"),
                    plot.type = 1, plot.params = plot.params,
                    add.cytobands.names=TRUE, 
                    cytobands.names.cex=0.25,
                    add.base.numbers=TRUE
      ) %>%
      kpDataBackground(data.panel = 1)
@
